...
export default {
    ...
    upload(uploadFile, options = {}) {
        ...
        if (options.chunk && file.size > options.chunkSize) {
            return chunkUpload(options.url, options.name, file, options.progress, options.chunkSize)
        }
        return singleUpload(options.url, options.name, file, options.progress)
    }
}
